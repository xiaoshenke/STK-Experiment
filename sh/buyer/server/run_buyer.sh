...
if [ $# -lt 1 ]
then
	echo Usage: sh/buyer/server/run_buyer.sh sid [--day ] 
      	exit 2
fi
...
if [[ $is_single_id == "1" ]]
then
	echo python engine/observe/buyer/server/flush_server_id.py flush $type $day $time_str --mode $mode
	python engine/observe/buyer/server/flush_server_id.py flush $type $day $time_str --mode $mode
