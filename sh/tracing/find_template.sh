...
if [ $# -lt 1 ]
then
	echo Usage: sh/tracing/find_template.sh template [--day ]
	exit 2
fi
...
if [ ${#result} -eq 0 ]
then
	echo 尝试寻找$cur_dir/engine/observe/tracing/template/ 下数据失败
else
	echo ""
	echo "## 默认的模板文件 find $cur_dir/engine/observe/tracing/template/ | grep $template"
	echo 成功找到文件如下:
	echo "$result"
	echo ""
fi
